...
Neural codec: compress waveforms → discrete latents → reconstruct. Target **<1 kbps** speech.

Repo root (this tree). Stack: **Python / PyTorch**. Device: **CUDA → MPS (Apple Silicon) → CPU** via `pick_device()` in `train_pipeline.py` / `train.py`. Primary GPU in past runs: RTX 3090 24GB.
...
SEANet + various decoders (Mamba / Zipformer / HiFi-GAN / Vocos scratch) → PESQ ~1.0 noise. **Cause**: 320× downsample kills detail. **Working path**: pretrained mel + **Vocos** (or separate HiFi-GAN experiments in `train.py` / pipeline).
...
Use `python3 run.py …` (see repo `README.md`).

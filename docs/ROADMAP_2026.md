...
**Numbers (70k, LibriSpeech)**: PESQ ~1.37 **no VQ** / ~1.31 **with VQ @500 bps**; VQ util ~3.3%; SI-SDR ~-25 / -27 dB; ~6.53M params; ~85 ms/step RTX 3090.

**Bottleneck**: Encoder–decoder ceiling ~**PESQ 1.37** at infinite bitrate — not quantizer (VQ cost ~0.06 PESQ). More steps / bigger codebooks won’t reach ~3.5+ alone.
...
Replace late encoder / decoder conv stacks with **Mamba-2** where temporal structure helps; bidirectional at bottleneck, causal at decode. Est. +0.8–1.2 PESQ continuous; +15–20% params; **high** integration cost.

### 2 — Dynamic FF + windows (medium–high)

Per-frame gating (2×→3× FF), RoPE/xPos on local attention; plosives / onsets. +0.3–0.5 PESQ est.; **medium** cost.

### 3 — 4-bit Q + speculative decode (deploy)

4-bit weights, 8-bit activations at 50 Hz; tiny draft for speculation. ~3× memory; ~2.5× effective speed; **medium** cost.

### Quick wins

- SwiGLU, RMSNorm — low risk  
- Flash-style kernels on attention — low–medium  

---

## Phases

**Phase 1 (weeks 1–4)**: Mamba-2 at 50 Hz + SwiGLU/RMSNorm → target PESQ **2.2+** continuous.

...
class MultiResolutionSTFTLoss(nn.Module):
    ...
    def compute_stft_pairs(self, x_b1t, y_b1t):
        """One STFT bank per resolution; reuse for MRSTFT disc (same resolutions)."""
        x, y = x_b1t.squeeze(1), y_b1t.squeeze(1)
        ...
        for n_fft, hop in self.stft_configs:
            ...
            pairs.append((X[:, :, :min_t], Y[:, :, :min_t]))
        return pairs

    def loss_from_stft_pairs(self, pairs):
        total_loss = 0.0
        for X, Y in pairs:
    ...
    def forward(self, x, y, stft_pairs=None):
        """x: recon, y: target. Both [B, 1, T]"""
        if stft_pairs is None:
            stft_pairs = self.compute_stft_pairs(x, y)
        return self.loss_from_stft_pairs(stft_pairs)
# ──────────────────────────────────────────────
class STFTDiscriminator(nn.Module):
    ...
    def forward_from_mag(self, mag):
        """mag [B, 1, F, T] — paired with MultiResolutionSTFTLoss STFT bank."""
        feats = []
        h = mag
        for conv in self.convs:
            h = conv(h)
...
def _write_resume_state(exp: Path, ckpt_path: Path, step: int, total_steps: int) -> None:
    ...
    Paths are relative to repo root so resume_state.json is portable across machines.
    """
    repo_root = REPO_ROOT
    exp = exp.resolve()
    ckpt_path = ckpt_path.resolve()
    try:
        ck_rel = str(ckpt_path.relative_to(repo_root))
    except ValueError:
        ck_rel = str(ckpt_path)
    try:
        exp_rel = str(exp.relative_to(repo_root))
    except ValueError:
        exp_rel = str(exp)
    payload = {
        "step": step,
...
def run_training(
    ...
    *,
    ...
    for step in range(start_step, cfg.total_steps):
        ...
        stft_pairs = None
        if cfg.lambda_stft > 0 or use_adv:
            stft_pairs = mrstft_loss_fn.compute_stft_pairs(audio_recon_b1t, audio)

        loss_stft = torch.zeros((), device=device, dtype=mel_recon.dtype)
        if cfg.lambda_stft > 0:
            loss_stft = mrstft_loss_fn.loss_from_stft_pairs(stft_pairs)
        ...
        # Adversarial — reuse STFT bank from MRSTFT loss when both share resolutions
        loss_adv = 0.0
        loss_feat = 0.0
        if use_adv:
            if stft_pairs is not None:
                pred_mags_b1ft = [X.abs().unsqueeze(1) for X, Y in stft_pairs]
                tgt_mags_b1ft = [Y.abs().unsqueeze(1) for X, Y in stft_pairs]
                real_out, real_feats = disc.forward_from_mags(tgt_mags_b1ft)
                fake_out, fake_feats = disc.forward_from_mags(pred_mags_b1ft)
            else:
        ...
        # Discriminator backward (reuse real logits from G pass — no second disc(audio))
        if use_adv:
            real_for_d = [r.detach() for r in real_out]
            if stft_pairs is not None:
                fake_out, _ = disc.forward_from_mags(
                    [X.abs().unsqueeze(1).detach() for X, Y in stft_pairs]
                )
            else:

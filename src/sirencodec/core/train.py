...
try:
    ...
    print("WARNING: sirencodec.data_pipeline not found — falling back to synthetic data")
...
# ═══════════════════════════════════════
# Shared (n_fft, hop) triple for MRSTFT recon loss + MRSTFT discriminator so each
# waveform only gets one STFT stack per training step when both are enabled.
MRSTFT_RECON_DISC_RESOLUTIONS = (
    (1024, 120),
    (2048, 240),
    (512, 50),
)
...
class STFTDiscriminator(nn.Module):
    ...
    def forward_from_mag(self, mag):
        """Same as forward after STFT; mag [B, 1, F, T] — shares STFT with MRSTFT loss."""
        feats = []
        h = mag
        for conv in self.convs:
            h = conv(h)
...
class MultiResolutionSTFTDiscriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.discs = nn.ModuleList([
            STFTDiscriminator(n_fft=n, hop_length=h)
            for n, h in MRSTFT_RECON_DISC_RESOLUTIONS
        ])
    ...
    def forward_from_mags(self, mags_list):
        """mags_list[i]: [B, 1, F, T] per resolution — no STFT (paired with MRSTFT loss)."""
        outputs, all_feats = [], []
        for disc, mag in zip(self.discs, mags_list):
            out, feats = disc.forward_from_mag(mag)
            outputs.append(out)
            all_feats.extend(feats)
        return outputs, all_feats
...
class MultiResolutionSTFTLoss(nn.Module):
    def __init__(self, resolutions=None):
        super().__init__()
        self.resolutions = list(resolutions or MRSTFT_RECON_DISC_RESOLUTIONS)

    def compute_mags_pair(self, pred_b1t, target_b1t):
        """Returns per-resolution magnitude tensors [B, F, T] for pred and target."""
        pred = pred_b1t.squeeze(1)
        target = target_b1t.squeeze(1)
        min_len = min(pred.size(-1), target.size(-1))
        pred = pred[..., :min_len]
        target = target[..., :min_len]
        ...
        for n_fft, hop in self.resolutions:
            ...
            pred_mags.append(pred_mag[..., :min_frames])
            target_mags.append(target_mag[..., :min_frames])
        return pred_mags, target_mags

    def loss_from_mags(self, pred_mags, target_mags):
        loss = 0.0
        for pred_mag, target_mag in zip(pred_mags, target_mags):
            loss += (pred_mag - target_mag).norm(p="fro") / target_mag.norm(p="fro")
            loss += (pred_mag.log() - target_mag.log()).abs().mean()
        return loss / max(len(self.resolutions), 1)

    def forward(self, pred, target, pred_mags=None, target_mags=None):
        if pred_mags is None or target_mags is None:
            pred_mags, target_mags = self.compute_mags_pair(pred, target)
        return self.loss_from_mags(pred_mags, target_mags)
...
def _vq_util_from_encode_indices(indices, cfg, codec):
    """Approximate RVQ/FSQ utilization from encode() indices (no full forward)."""
    if cfg.use_fsq:
        return 1.0
    if indices is None:
        return 0.0
    if isinstance(indices, torch.Tensor):
        indices = [indices]
    if not indices or not isinstance(indices[0], torch.Tensor):
        return 0.0
    try:
        q = getattr(codec, "quantizer", None)
        total = 0.0
        for i, idx in enumerate(indices):
            n_codes = float(cfg.codebook_size)
            if q is not None:
                layers = getattr(q, "layers", None)
                if layers is not None and i < len(layers):
                    n_codes = float(layers[i].n_codes)
                elif getattr(q, "semantic_vq", None) is not None and i == 0:
                    n_codes = float(q.semantic_vq.n_codes)
                elif getattr(q, "acoustic_vq", None) is not None and i == 1:
                    n_codes = float(q.acoustic_vq.n_codes)
                elif getattr(q, "coarse_vq", None) is not None and i == 0:
                    n_codes = float(q.coarse_vq.n_codes)
                elif getattr(q, "fine_vq", None) is not None and i == 1:
                    n_codes = float(q.fine_vq.n_codes)
            total += idx.float().unique().numel() / max(n_codes, 1.0)
...
def evaluate_codec(
    ...
    *,
    ...
    SI-SDR and VQ utilization use encode→decode only (no redundant full forward).
    Set ``compute_pesq=True`` to run PESQ on CPU (slow).
    """
    codec.eval()
    metrics = {}
    ...
    with torch.no_grad():
        ...
        for _ in range(num_samples):
            ...
            if x_hat_eval.dim() == 2:
                x_hat_eval = x_hat_eval.unsqueeze(1)

            sdr = si_sdr(x_hat_eval, x)
            all_si_sdr.append(sdr)
            all_utils.append(_vq_util_from_encode_indices(indices, cfg, codec))
            n_evaluated += 1

            # PESQ: optional CPU metric (off by default for eval throughput)
            if compute_pesq and pesq_fn is not None:
                try:
                    ref = x.squeeze().cpu().numpy()
                    deg = x_hat_eval.squeeze().detach().cpu().numpy()
...
def train(cfg: CodecConfig):
    ...
    for step in range(cfg.total_steps):
        ...
        if mrstft_loss_fn:
            mr_pred_mags, mr_tgt_mags = mrstft_loss_fn.compute_mags_pair(x_hat, x)
        ...
        if opt_disc and step > cfg.warmup_steps:
            ...
            if mrstft_disc:
                ...
                loss_disc = adversarial_d_loss(real_for_d, fake_out)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "download":
        if not HAS_DATA_PIPELINE:
            print("ERROR: data_pipeline module not available. Install the package first.")
            sys.exit(1)

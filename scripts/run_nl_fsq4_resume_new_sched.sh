#!/usr/bin/env bash
# Resume weights from ckpt, but new LR schedule + adv warmup + optional fresh Adam + segment ramp from 0.
# Same pick logic as continue_long: RESUME or OUT/resume_state.json or newest ckpt or INITIAL_CKPT.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if explicit:
    p = Path(explicit)
    if not p.is_file():
        print("RESUME not a file:", explicit, file=sys.stderr)
        sys.exit(1)
...
RESET_OPT=(--reset-lr-schedule)
if [[ "${FRESH_ADAM:-1}" == "1" ]]; then
  RESET_OPT+=(--reset-optimizer-on-resume)
fi
if [[ "${RESET_SEG:-1}" == "1" ]]; then
  RESET_OPT+=(--reset-segment-curriculum)
fi

echo "[resume+new_sched] ckpt=$RESUME_CKPT → OUT=$OUT STEPS=$STEPS flags=${RESET_OPT[*]}"
echo "[resume+new_sched] lambda_stft=$LAMBDA_STFT lambda_mel_q=$LAMBDA_MEL_Q"

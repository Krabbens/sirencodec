...
import json, math, random, subprocess, csv, time, hashlib
...
def download_librispeech(data_dir: str, subsets: list, max_datasets=None):
    ...
    for subset in subsets:
        ...
        if not tar_path.exists():
            _run(f"wget -O {tar_path} {urls[subset]} -q --show-progress")
...
def download_vctk(data_dir: str, max_datasets=None):
    ...
    try:
        _run(f"wget -O {zip_path} {url} -q --show-progress")
    except RuntimeError:
        print("[VCTK] Primary URL failed, trying alternate...")
        _run(f"wget -O {zip_path} {alt_url} -q --show-progress")

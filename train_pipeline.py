...
# Debug logging for runtime evidence (session cc2f00).
DEBUG_LOG_PATH = Path("/home/sperm/autoresearch/sirencodec/.cursor/debug-cc2f00.log")
...
def run_stage(codec, cfg, device, start_step, end_step,
    ...
    for step in range(start_step, end_step):
        ...
        if mel_loss_fn:
            ...
            mel_tgt = torchaudio.transforms.MelSpectrogram(
                sample_rate=cfg.sample_rate, n_fft=1024, hop_length=cfg.hop_length
            )(wave.squeeze(1))
            loss_mel = F.l1_loss(mel_pred.log().clamp(min=-10), mel_tgt.log().clamp(min=-10))

        # MRSTFT loss (reconstruction)
        loss_mrstft = mrstft_loss_fn(x_hat, wave) if mrstft_loss_fn else 0.0
        ...
        use_adv = cfg.use_mrstft and mrstft_disc and step > cfg.warmup_steps
        ...
        if step % cfg.eval_every == 0 and step > start_step and not skip_eval:
            sys.stdout.write("\n")
            from train import evaluate_codec
            codec.eval()
